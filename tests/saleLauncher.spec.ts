...
describe('SaleLauncher', function() {
  ...
  test('Should save new FixedPriceSale', async () => {
    ...
    expect(data.data.fixedPriceSale.tokenIn.id).toMatch(biddingToken.address)
    ...
    expect(data.data.fixedPriceSale.tokenOut.id).toMatch(fixedPriceSaleToken.address)
